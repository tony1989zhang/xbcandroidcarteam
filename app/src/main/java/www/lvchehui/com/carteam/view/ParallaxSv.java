...
/**
 * 作者：V先生 on 2016/8/2 19:31
 * 作用：滑动图片放大缩小Scrollview
 */
public class ParallaxSv extends ScrollView implements OnScrollListener {
    ...
    private interface OnOverScrollByListener {
         boolean overScrollBy(int deltaX, int deltaY, int scrollX, int scrollY, int scrollRangeX, int scrollRangeY, int maxOverScrollX, int maxOverScrollY,
    ...
    public ParallaxSv(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(context);
    }

    public ParallaxSv(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public ParallaxSv(Context context) {
        super(context);
        init(context);
    }

    public void init(Context context) {
        mDefaultImageViewHeight = context.getResources().getDimensionPixelSize(R.dimen.parallax_ad_height_size);
        originImageViewHeight = context.getResources().getDimensionPixelSize(R.dimen.parallax_ad_height_size);
    }
